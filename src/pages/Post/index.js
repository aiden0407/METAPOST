    ...
    for (const depth0 of depth0Comments) {
      for (const depth1 of depth1Comments) {
        if (depth1.parent_id === depth0.id) {
          depth0Comments.splice(
            depth0Comments.indexOf(
              depth0Comments.find((c) => c.id === depth0.id)
            ) + 1,
            0,
            depth1
          );
    ...
    >
      <CommunityBox>
        {postDetail.detail[0].community_title && (
          <Row
            onClick={() => {
              if (postDetail.detail[0]?.community_id) {
                navigate(
                  `/community?community_id=${postDetail.detail[0]?.community_id}`
                );
              }
            }}
          ...
          >
            <Image
              src={postDetail.detail[0]?.community_logo_url ?? defaultCommunity}
              width={16}
              height={16}
              borderRadius="2px"
      ...
      <TitleBox>
        ...
        >
          {postDetail.detail[0].title}
        </Text>
      </TitleBox>

      <ContentBox>
        <Row style={{ width: "100%", position: "relative" }}>
          <Text B3 color={COLOR.N600}>
            {formatDateTime(postDetail.detail[0].created_at)}
          </Text>
          <FlexBox />
          <Text B2 color={COLOR.N600}>
            {formatNumber(Math.round(postDetail.detail[0].viewer_count / 2))}
            &nbsp;views
          </Text>
          <StyledImage
            src={moreIcon}
            width={16}
            marginLeft={8}
            onClick={() => setIsToggleOpened(true)}
          />
          {isToggleOpened && (
            <ToggleBox>
              {postDetail.detail[0].nickname === loginData?.user?.nickname && (
                <>
                  <StyledRow marginLeft={8} onClick={() => handleEditPost()}>
                    ...
                    <Text B3 color={COLOR.N700} marginLeft={8}>
                      Edit
                    </Text>
                  </StyledRow>
        ...
        <Row marginTop={18}>
          <Row
            ...
            onClick={() => {
              if (postDetail.detail[0]?.user_id === loginData?.user?.id) {
                ...
                navigate(
                  `/profile?profile_id=${postDetail.detail[0]?.user_id}`
                );
                window.scrollTo({ top: 0 });
              }
            }}
          >
            <Image
              src={postDetail.detail[0]?.nft_thumbnail ?? defaultProfile}
              width={33}
              borderRadius="4px"
            />
            <Column marginLeft={8} gap={4}>
              <Text B2 medium color={COLOR.N700}>
                {postDetail.detail[0].nickname}
              </Text>
              <Row>
                <Image
                  src={postDetail.detail[0].nft_title ? nftIcon : nftNoneIcon}
                  width={16}
                />
                {postDetail.detail[0].nft_title ? (
                  <Text B3 medium color={COLOR.N700} marginLeft={4}>
                    {postDetail.detail[0].nft_title}
                  </Text>
                ) : (
                  <Text B3 medium color={COLOR.N600} marginLeft={4}>
                    None
                  </Text>
                )}
              </Row>
        ...
        <DescriptionBox
          ...
          dangerouslySetInnerHTML={{
            __html: linkifyUrls(postDetail.detail[0]?.description),
          }}
        />
        ...
        >
          <LongShortButton onClick={() => handleLikedPost(true)}>
            ...
            <LongShortCount B2 medium color="#26CA5E">
              {postDetail.detail[0].liked_count}
            </LongShortCount>
          </LongShortButton>
          <LongShortButton onClick={() => handleLikedPost(false)}>
            ...
            <LongShortCount B2 medium color={COLOR.BLUE1}>
              {postDetail.detail[0].disliked_count}
            </LongShortCount>
          </LongShortButton>
        </Row>
      </ContentBox>
      ...
      <CommentBox>
        <Row>
          ...
          <Text B2 medium color={COLOR.N800} marginLeft={8}>
            All Comments
          </Text>
          <Text B2 medium color={COLOR.N700} marginLeft={8}>
            {maxLength ?? 0}
          </Text>
        </Row>
